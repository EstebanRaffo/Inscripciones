...
public class ReasignarDocente extends JInternalFrame {
	...
	private void configurar() {
		...
		List<ProfesorView> profesores = ControladorNegocio.getInstancia().getProfesores();
		...
		btnAceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(datosValidos()){
					ControladorNegocio.getInstancia().reasignarDocente(((CursoView)cboCursos.getSelectedItem()).getNumero(), ((ProfesorView)cboProfesores.getSelectedItem()).getLegajo());
					dispose();
				}
				else
